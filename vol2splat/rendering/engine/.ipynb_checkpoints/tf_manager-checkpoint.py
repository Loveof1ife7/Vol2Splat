...
class TFManager:
    ...
    def _build_sum2_partner_map(self, n_bands: int) -> dict[int, int]:
        partner = {}
        if n_bands <= 1:
            if n_bands == 1:
                partner[1] = 1
        ...
        for idx in range(1, n_bands + 1):
            candidates = [j for j in range(1, n_bands + 1) if j != idx and abs(j - idx) != 1]
            if not candidates:
                candidates = [j for j in range(1, n_bands + 1) if j != idx]
            partner[idx] = rng.choice(candidates)
        return partner
    ...
    def make_tf_points(self,idx: int,n_b: int,arr_min: float,arr_max: float,
        ...
        def _band_range(band_idx: int) -> tuple[float, float]:
            if band_edges:
                return float(band_edges[band_idx - 1]), float(band_edges[band_idx])
            span = float(arr_max - arr_min) if arr_max > arr_min else 1.0
            band_w = span / float(n_b)
            low_local = float(arr_min) + float(band_idx - 1) * band_w
        ...
        # 3. --- 应用 opacity_scale 缩放因子（与 gs-datagen 实现保持一致） ---
        opacity_scale = getattr(self.args, "opacity_scale", 1.0)
        if opacity_scale != 1.0:
            scaled_pts_tuples = []
            for x, a in pts_tuples:
                scaled_pts_tuples.append((x, a * opacity_scale))
            pts_tuples = scaled_pts_tuples
            print(f"[info] Applied opacity_scale={opacity_scale:.3f} to TF band {idx}")

        # 4. --- 去除重复点并转换为ParaView格式 ---
        # (此逻辑不变)
        final_pts = []
        last_x = -float('inf')
        for x, a in pts_tuples:
