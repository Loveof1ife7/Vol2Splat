...
def _analyze_tf_output(tf_output: Dict[str, Any], qc_cfg: Dict[str, Any]) -> Dict[str, Any]:
...
def _write_qc_markdown(path: str, report: Dict[str, Any]) -> None:
    lines = [
        ...
        "",
        "| TF | Status | Mean | Nonzero | Std | Reasons |",
        "| --- | --- | ---: | ---: | ---: | --- |",
    ]
    for item in report["items"]:
        ...
        lines.append(
            ...
            f"{metrics['intensity_std']:.4f} | {reasons} |"
        )
...
def run_render_qc(render_result: Dict[str, Any], qc_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    items = [_analyze_tf_output(item, qc_cfg) for item in tf_outputs]
